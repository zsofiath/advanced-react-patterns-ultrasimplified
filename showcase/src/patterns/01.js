...
const MediumClap = ({ animationTimeline }) => {
  ...
  const handleClapClick = () => {
    ...
    setClapState(previousState => ({
      count: Math.min(previousState.count + 1, MAXIMUM_USER_CLAP),
      countTotal: count < MAXIMUM_USER_CLAP ? previousState.countTotal + 1 : previousState.countTotal,
      isClicked: true
    }))
  }
...
const Usage = () => {
  const AnimatedClap = withClapAnimation(MediumClap)
  return <AnimatedClap />
}
